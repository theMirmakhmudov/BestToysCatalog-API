...
def send_email_sync(to_email: str, subject: str, html_body: str):
    ...
    msg["Subject"] = subject
    msg["From"] = formataddr((settings.SMTP_FROM_NAME or "Obidov Toys System", settings.SMTP_FROM))
    msg["To"] = to_email
...
def build_welcome_email(customer_name: str, phone: str, email: str, password: str) -> tuple[str, str]:
    subject = "Welcome to Obidov Toys System — Your Account Password"
    html = f"""
    <div style="font-family:Inter,Arial,sans-serif;max-width:600px;margin:auto;">
      <h2 style="color:#222;">Salom, {customer_name}!</h2>
      <p>Siz Obidov Toys System’da muvaffaqiyatli ro‘yxatdan o‘tdingiz.</p>
      <p><b>Kirish ma’lumotlari:</b></p>
      <ul>
        <li>Telefon raqam: <b>{phone}</b></li>
        <li>Email: <b>{email}</b></li>
        <li>Parol: <b>{password}</b></li>
      </ul>
      <p>Xavfsizlik uchun ushbu parolni hech kimga bermaslikni tavsiya qilamiz.</p>
      <hr>
